...
describe('saveReport', () => {
  ...
  it('submits report data before generating and uploading the PDF', async () => {
    const calls = [];
    const report = {
      reportId: 'RPT-001',
      answers: { Q0_NAME: '测试用户' },
    ...
    const createPdf = vi.fn(async () => {
      calls.push('createPdf');
      return { blob: new Blob(['pdf']), filename: 'RPT-001.pdf' };
    });
    const uploadPdf = vi.fn(async () => {
      calls.push('uploadPdf');
    ...
    expect(calls).toEqual(['submit', 'createPdf', 'uploadPdf']);
    expect(result).toMatchObject({ success: true, recordSaved: true, pdfUploaded: true });
