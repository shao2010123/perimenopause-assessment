...
const MAX_UPLOAD_HTML_BYTES = 5 * 1024 * 1024;
...
export async function uploadReportHtml({ reportId, recordId, htmlBlob, fileName }) {
  if (!recordId || !htmlBlob || typeof fetch !== 'function') {
    return { success: false, error: '缺少飞书记录或 HTML 文件' };
  }

  if (Number(htmlBlob.size) > MAX_UPLOAD_HTML_BYTES) {
    return { success: false, error: 'HTML 文件过大，请重新生成后再试' };
  }

  try {
    const htmlBase64 = await blobToBase64(htmlBlob);
    const response = await fetch('/api/upload-report-html', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        recordId,
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'HTML 上传失败',
    };
  }
}
...
export async function submitReportThenUploadPdf({
  ...
  try {
    ...
    return {
      ...
      pdfUploaded: true,
      recordId,
      reportFileName: pdf?.filename,
      reportOpenUrl: pdf?.openUrl,
    };
  } catch (error) {
    return {
      success: false,
