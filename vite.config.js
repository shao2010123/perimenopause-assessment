...
import { submitReportHandler, uploadReportHtmlHandler, uploadReportPdfHandler } from './server/apiHandler.mjs';
...
function feishuSubmitApi() {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/upload-report-pdf', (request, response) => {
        uploadReportPdfHandler(request, response);
      });
      server.middlewares.use('/api/upload-report-html', (request, response) => {
